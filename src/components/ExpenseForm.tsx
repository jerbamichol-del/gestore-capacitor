  ...
  return (
    <>
      <div className={`fixed inset-0 z-[5000] transition-opacity duration-300 ease-in-out ${isAnimating ? 'opacity-100' : 'opacity-0'} bg-slate-900/80 backdrop-blur-md`} onClick={handleBackdropClick} aria-modal="true" role="dialog">
        <div className={`bg-[#FFF8F0] dark:bg-midnight w-full h-full flex flex-col absolute bottom-0 transform transition-transform duration-300 ease-in-out ${isAnimating ? 'translate-y-0' : 'translate-y-full'}`} onClick={(e) => e.stopPropagation()} style={{ touchAction: 'pan-y' }}>
          ...
          <form onSubmit={handleSubmit} noValidate className="flex-1 flex flex-col overflow-hidden">
            ...
            <footer className={`px-6 py-4 bg-[#FFF8F0] dark:bg-midnight border-t border-slate-200 dark:border-electric-violet/20 flex flex-shrink-0 ${isEditing && !hasChanges ? 'justify-stretch' : 'justify-end gap-3'}`}>
        ...
        {isRecurrenceModalOpen && (
          <div className={`fixed inset-0 z-[60] flex justify-center items-center p-4 transition-opacity duration-300 ease-in-out ${isRecurrenceModalAnimating ? 'opacity-100' : 'opacity-0'} bg-slate-900/80 backdrop-blur-md`} onClick={handleCloseRecurrenceModal} aria-modal="true" role="dialog">
            <div className={`midnight-card rounded-lg shadow-xl w-full max-w-sm border border-slate-200 dark:border-electric-violet/30 transform transition-all duration-300 ease-in-out ${isRecurrenceModalAnimating ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`} onClick={(e) => e.stopPropagation()}>
              ...
              <footer className="p-4 bg-[#FFF8F0] dark:bg-midnight border-t border-slate-200 dark:border-electric-violet/20 flex justify-end"><button type="button" onClick={handleApplyRecurrence} className="px-4 py-2 text-sm font-bold text-white btn-electric rounded-lg shadow-lg hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-all">Applica</button></footer>
            </div>
          </div>
        )}
