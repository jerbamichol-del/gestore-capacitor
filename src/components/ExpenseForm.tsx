            ...
            <div className="p-6 space-y-4 flex-1 overflow-y-auto">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><FormInput ref={amountInputRef} id="amount" name="amount" label="Importo" value={formData.amount || ''} onChange={handleInputChange} onKeyDown={handleAmountEnter} icon={<CurrencyEuroIcon className="h-5 w-5 text-slate-400" />} type="text" inputMode="decimal" pattern="[0-9]*[.,]?[0-9]*" placeholder="0.00" required autoComplete="off" />
                <div className={`grid ${formData.frequency === 'recurring' ? 'grid-cols-1' : 'grid-cols-2'} gap-2`}>
                  <div>
                    <label htmlFor="date" className="block text-base font-medium text-slate-700 dark:text-slate-300 mb-1">{isSingleRecurring ? 'Data del Pagamento' : formData.frequency === 'recurring' ? 'Data di Inizio' : 'Data'}</label>
                    <div className="relative">
                      <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                        <CalendarIcon className="h-5 w-5 text-slate-400" />
                      </div>
                      <input id="date" name="date" value={formData.date || ''} onChange={handleInputChange} className="block w-full rounded-md border border-slate-300 dark:border-electric-violet/30 bg-white dark:bg-midnight-card py-2.5 pl-10 pr-3 text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:border-indigo-500 dark:focus:border-electric-violet focus:ring-1 focus:ring-indigo-500 dark:focus:ring-electric-violet text-base" type="date" />
                    </div>
                  </div>
                  {formData.frequency !== 'recurring' && (
                    <div>
                      <label htmlFor="time" className="block text-base font-medium text-slate-700 dark:text-slate-300 mb-1">Ora (opz.)</label>
                      <div className="relative">
                        <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                        ...
                        <input
                          ...
                          className="block w-full rounded-md border border-slate-300 dark:border-electric-violet/30 bg-white dark:bg-midnight-card py-2.5 pl-10 pr-3 text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:border-indigo-500 dark:focus:border-electric-violet focus:ring-1 focus:ring-indigo-500 dark:focus:ring-electric-violet text-base [color-scheme:light] dark:[color-scheme:dark]"
                          type="time"
                        />
                      </div>
                    </div>
              ...
              {isForRecurringTemplate && !isIncome && !isTransfer && (
                <div className="bg-white dark:bg-midnight-card p-4 rounded-xl border-2 border-slate-200 dark:border-electric-violet/40 shadow-lg space-y-4 ring-1 ring-slate-100 dark:ring-transparent transition-all">
                  <div>
                    <label className="block text-base font-bold text-slate-800 dark:text-white mb-2 ml-1">Frequenza</label>
                    <button type="button" onClick={() => setActiveMenu('frequency')} className="w-full flex items-center justify-between text-left gap-2 px-4 py-3 text-base rounded-xl border border-slate-300 dark:border-electric-violet/50 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:focus:ring-electric-violet transition-colors bg-slate-50 dark:bg-midnight-card/80 text-slate-800 dark:text-white hover:bg-white dark:hover:bg-midnight transition-all">
                      <span className="truncate flex-1 font-semibold capitalize">{isSingleRecurring ? 'Singolo' : formData.frequency !== 'recurring' ? 'Nessuna' : 'Ricorrente'}</span>
                      <ChevronDownIcon className="w-5 h-5 text-indigo-500 dark:text-electric-violet" />
                    </button>
                  </div>
                  {formData.frequency === 'recurring' && !isSingleRecurring && (
                    <div className="animate-fade-in-up">
                      <label className="block text-base font-bold text-slate-800 dark:text-white mb-2 ml-1">Ricorrenza</label>
                      <button type="button" onClick={() => setIsRecurrenceModalOpen(true)} className="w-full flex items-center justify-between text-left gap-2 px-4 py-3 text-base rounded-xl border border-slate-300 dark:border-electric-violet/50 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:focus:ring-electric-violet transition-colors bg-slate-50 dark:bg-midnight-card/80 text-slate-800 dark:text-white hover:bg-white dark:hover:bg-midnight transition-all">
                        <span className="truncate flex-1 font-semibold">{getRecurrenceSummary(formData as Partial<Expense>)}</span>
                        <ChevronDownIcon className="w-5 h-5 text-indigo-500 dark:text-electric-violet" />
                      </button>
                    </div>
                  )}
