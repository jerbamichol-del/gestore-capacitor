...
interface NotificationPermissionModalProps {
  ...
  onRefreshClick?: () => Promise<void> | void; // ✅ NEW: Manual refresh callback
}
...
export function NotificationPermissionModal({
  ...
  const handleRefreshClick = async () => {
    if (!onRefreshClick) return;
    
    setIsRefreshing(true);
    try {
      await onRefreshClick();
      // If permission is now enabled, the modal will close automatically
      // because the parent component will set isOpen=false
    } catch (e) {
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1">
                ...
                <ol className="text-xs text-amber-800 space-y-1 list-decimal list-inside">
                  ...
                  <li>Torna qui e clicca <strong>"Aggiorna"</strong></li>
                </ol>
              </div>
            </div>
          </div>
        ...
        <div className="px-4 pb-4 pt-2 flex-shrink-0 border-t border-gray-100">
          ...
          {onRefreshClick && (
            <button
              ...
              className="w-full px-3 py-2.5 mb-2 bg-green-500 hover:bg-green-600 disabled:bg-gray-300 text-white text-sm font-semibold rounded-lg shadow-lg transition-all flex items-center justify-center gap-2"
            >
              {isRefreshing ? (
                <>
