...
const MultipleExpensesModal: React.FC<MultipleExpensesModalProps> = ({ isOpen, onClose, expenses, accounts, onConfirm }) => {
  ...
  return (
    ...
    >
      <div
        className={`bg-white dark:midnight-card rounded-lg shadow-xl w-full max-w-3xl transform transition-all duration-75 ease-in-out ${isAnimating ? 'scale-100 opacity-100' : 'scale-95 opacity-0'} border border-transparent dark:border-electric-violet/30`}
          ...
          <div className="space-y-3">
            {editableExpenses.map((expense, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`bg-white dark:midnight-card rounded-lg shadow-sm border ${isSelected ? 'border-indigo-400 dark:border-electric-violet' : 'border-slate-200 dark:border-electric-violet/20'} transition-all duration-300 animate-fade-in-up`}
