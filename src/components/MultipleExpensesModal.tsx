...
const CustomCheckbox = ({ checked, onChange, id, label }: { checked: boolean, onChange: () => void, id: string, label: string }) => (
  <div className="flex items-center">
    ...
    />
    <label htmlFor={id} className="ml-2 text-sm font-medium text-slate-700 dark:text-slate-300 sr-only transition-colors">
      {label}
    </label>
  </div>
);
...
const SelectionButton = ({ label, value, onClick, placeholder, ariaLabel, disabled, icon }: { label: string, value?: string, onClick: () => void, placeholder: string, ariaLabel: string, disabled?: boolean, icon: React.ReactNode }) => {
  ...
  return (
    <div>
      <label className={`block text-sm font-medium mb-1 transition-colors ${disabled ? 'text-slate-400 dark:text-slate-600' : 'text-slate-700 dark:text-slate-300'}`}>{label}</label>
      <button
        type="button"
        onClick={onClick}
...
const MultipleExpensesModal: React.FC<MultipleExpensesModalProps> = ({ isOpen, onClose, expenses, accounts, onConfirm }) => {
  ...
  return (
    <div
      className={`fixed inset-0 z-[5200] flex justify-center items-center p-4 transition-opacity duration-75 ease-in-out ${isAnimating ? 'opacity-100' : 'opacity-0'} bg-slate-900/60 backdrop-blur-sm overflow-y-auto transition-colors`}
      onClick={onClose}
      aria-modal="true"
      role="dialog"
    >
      <div
        className={`bg-slate-50 dark:bg-slate-950 rounded-lg shadow-xl w-full max-w-3xl transform transition-all duration-75 ease-in-out ${isAnimating ? 'scale-100 opacity-100' : 'scale-95 opacity-0'} transition-colors`}
        onClick={(e) => e.stopPropagation()}
        style={{ maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}
      >
        <div className="flex justify-between items-center p-6 border-b border-slate-200 dark:border-slate-800 sticky top-0 bg-slate-50/80 dark:bg-slate-950/80 backdrop-blur-sm rounded-t-lg z-20 flex-shrink-0 transition-colors">
          <div>
            <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 transition-colors">Spese Rilevate</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400 transition-colors">Abbiamo trovato {expenses.length} spese. Seleziona e modifica i dettagli prima di aggiungerle.</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto flex-1 transition-colors">
          <div className="flex items-center bg-slate-100 dark:bg-slate-900 p-2 rounded-md mb-4 border border-slate-200 dark:border-slate-800 transition-colors">
            <CustomCheckbox
              id="select-all"
            ...
            />
            <label htmlFor="select-all" className="ml-3 text-sm font-medium text-slate-700 dark:text-slate-300 cursor-pointer transition-colors">
          ...
          <div className="space-y-3">
            {editableExpenses.map((expense, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`bg-white dark:bg-slate-900 rounded-lg shadow-sm border ${isSelected ? 'border-indigo-400 dark:border-indigo-600' : 'border-slate-200 dark:border-slate-800'} transition-all duration-300 animate-fade-in-up`}
                ...
                >
                  <div className="p-3 flex items-center gap-3">
                    ...
                    <input
                      ...
                      max={today}
                      className="text-sm rounded-md border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 py-1.5 px-2 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors"
                    />
                    <div className="flex-grow" />
                    <p className="text-lg font-bold text-indigo-600 dark:text-indigo-400 shrink-0 transition-colors">
                      {formatCurrency(expense.amount || 0)}
                    </p>
                    <button onClick={() => handleToggleExpand(index)} className="p-2 text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-900/30 rounded-full transition-colors flex-shrink-0" aria-label="Modifica dettagli spesa">
                      <PencilSquareIcon className="w-5 h-5" />
                    </button>
                  </div>

                  {isExpanded && (
                    <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-800/50 space-y-4 transition-colors">
                      <div>
                        <label htmlFor={`description-${index}`} className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1 transition-colors">Descrizione</label>
                        <div className="relative">
                          <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                            <DocumentTextIcon className="h-5 w-5 text-slate-400 dark:text-slate-500" aria-hidden="true" />
                          </div>
                          <input
                            type="text"
                            ...
                            className="block w-full rounded-md border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 py-2 pl-10 pr-3 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 sm:text-sm transition-colors"
                      ...
                      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                        <SelectionButton
                          ...
                          icon={<CreditCardIcon className="h-5 w-5 text-slate-400 dark:text-slate-500" />}
                        />
                        <SelectionButton
                          label="Categoria"
                          ...
                          icon={<TagIcon className="h-5 w-5 text-slate-400 dark:text-slate-500" />}
                        />
                        <SelectionButton
        ...
        <div className="px-6 py-4 bg-slate-100 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 flex justify-end gap-3 sticky bottom-0 rounded-b-lg flex-shrink-0 transition-colors">
          <button
            type="button"
            onClick={onClose}
          ...
          <button
            ...
            className="px-4 py-2 text-sm font-bold text-white bg-indigo-600 dark:bg-indigo-500 rounded-lg shadow-sm hover:bg-indigo-700 dark:hover:bg-indigo-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-colors disabled:bg-indigo-300 dark:disabled:bg-indigo-900/50 disabled:cursor-not-allowed"
          >
