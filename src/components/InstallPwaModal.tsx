...
const InstallPwaModal: React.FC<InstallPwaModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    ...
    >
      <div
        className={`midnight-card rounded-lg shadow-xl w-full max-w-lg transform transition-all duration-300 ease-in-out ${isAnimating ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex justify-between items-center p-6 border-b border-slate-200">
          <h2 className="text-xl font-bold text-sunset-text dark:text-white">Installa l'App</h2>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-700 p-1 rounded-full hover:bg-slate-200">
            <XMarkIcon className="w-6 h-6" />
          </button>
        </div>
        <div className="p-6 space-y-4">
          <div className="text-sm text-slate-600 dark:text-slate-400 bg-sunset-cream/60 dark:bg-midnight-card/50 p-3 rounded-md flex items-start gap-2">
          ...
          <div className="midnight-card p-4 rounded-lg border border-sunset-coral/20 dark:border-electric-violet/20">
            ...
            <div className="flex gap-2">
              <input
                ...
                readOnly
                className="flex-grow bg-sunset-cream/60 dark:bg-midnight-card/50 text-slate-700 dark:text-white rounded-md border border-slate-300 dark:border-electric-violet/30 px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:focus:ring-electric-violet"
              />
              <button
                onClick={handleCopy}
          ...
          <div className="bg-sunset-peach/40 dark:bg-electric-violet/20 p-4 rounded-lg border border-sunset-coral/50 dark:border-electric-violet/30">
            <p className="font-bold text-sunset-coral dark:text-electric-violet mb-2">3. Aggiungi alla Home</p>
