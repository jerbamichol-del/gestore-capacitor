  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="absolute bottom-0 left-0 right-0 z-10 bg-slate-50 dark:bg-slate-900 rounded-t-2xl shadow-xl max-h-[80vh] flex flex-col transition-colors"
        style={{
        ...
        {...tapBridgeHandlers}
      >
        <header className="flex justify-between items-center p-4 border-b border-slate-200 dark:border-slate-800 flex-shrink-0 transition-colors">
          <div className="flex-1 text-center">
            <div className="inline-block h-1.5 w-10 rounded-full bg-slate-300 dark:bg-slate-700 absolute top-2 left-1/2 -translate-x-1/2" />
            <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100 pointer-events-none mt-2 transition-colors">{title}</h2>
          </div>
          <button
            type="button"
            onClick={handleManualClose}
            className="text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 transition-colors p-2 rounded-full hover:bg-slate-200 dark:hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 absolute top-2 right-2"
            aria-label="Chiudi"
          >
              ...
              return (
                <li key={option.value}>
                  <button
                    ...
                    className={`w-full text-left p-4 flex items-center justify-between gap-4 transition-colors rounded-lg ${isSelected ? 'bg-indigo-100 dark:bg-indigo-900/40' : 'hover:bg-slate-200 dark:hover:bg-slate-800'
                      }`}
                  >
                    <span className="flex items-center gap-4 min-w-0">
                      ...
                      <span className={`font-medium text-lg truncate transition-colors ${isSelected ? 'text-indigo-700 dark:text-indigo-300' : 'text-slate-800 dark:text-slate-200'}`}>
                        {option.label}
                      </span>
                    </span>
                    {isSelected && <CheckCircleIcon className="w-7 h-7 text-indigo-600 dark:text-indigo-400 flex-shrink-0" />}
                  </button>
                </li>
              );
            })}
