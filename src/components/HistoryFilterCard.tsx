...
export const CustomDateRangeInputs: React.FC<{
  ...
  return (
    <div
      className={
        `w-full ${compact ? 'h-8' : 'h-10'} flex border rounded-lg overflow-hidden transition-colors relative border-sunset-pink/20 dark:border-electric-violet/50 bg-[rgb(224,239,222)] dark:bg-electric-violet/20`
      }
    >
    ...
    return (
      ...
      >
        ...
        <button
          ...
          type="button"
          className={
            `flex-1 h-full text-sm font-semibold bg-[rgb(224,239,222)] dark:bg-electric-violet/20 text-black dark:text-electric-violet ` +
            (compact ? '' : ' hover:bg-[rgb(204,219,202)] dark:hover:bg-electric-violet/30 transition-colors')
          }
          data-no-drag
          {...tapBridge}
        >
...
const IntegratedFilterHeader: React.FC<{ isPanelOpen: boolean }> = ({ isPanelOpen }) => {
  ...
  const R = 16; // Match rounded-t-2xl (16px)
  const tabW = 88;
  const tabH = 19;
  const bulge = 22;
  const plateau = 30.8;
  ...
  // Una singola forma che copre sia la linguetta che il bordo superiore della card
  // per assicurare che il colore sia identico e non ci siano intercapedini.
  const headerFillPath = [
    `M 0 ${R}`,
    `Q 0 0 ${R} 0`,
    `L ${x1} 0`,
    `C ${x1 + bulge} 0, ${tx1} -${tabH}, ${tx1} -${tabH}`,
    `L ${tx2} -${tabH}`,
    `C ${tx2 + bulge} -${tabH}, ${x2 - bulge} 0, ${x2} 0`,
    `L ${width - R} 0`,
    `Q ${width} 0 ${width} ${R}`,
    `L ${width} 10`, // scende un po' dentro la card per sovrapporsi
    `L 0 10`,
    `Z`
  ].join(' ');
    ...
    <div className="absolute top-0 left-0 w-full h-0 pointer-events-none z-50">
      ...
      >
        ...
        <g className="dark:hidden" filter="url(#header-shadow-light)">
          <path d={headerFillPath} fill="#F2F4F2" className="pointer-events-auto cursor-grab" />
        ...
        <g className="hidden dark:block" filter="url(#header-shadow-dark)">
          <path d={headerFillPath} fill="#0F172A" className="pointer-events-auto cursor-grab" />
  ...
  const renderAccountSelection = () => (
    <div className="space-y-2">
      <div className="flex items-center gap-2 mb-3 pb-2 border-b border-slate-100 dark:border-slate-800/60">
        ...
        <h3 className="text-base font-bold text-slate-800 dark:text-white">Seleziona Conto</h3>
      </div>
            ...
            <div key={cat} className="rounded-lg overflow-hidden border border-transparent hover:border-slate-100 dark:hover:border-electric-violet/20">
              <div className={`w-full flex items-center px-3 py-2 gap-3 transition-colors ${isParentVisuallyChecked ? 'bg-indigo-50 dark:bg-electric-violet/20' : 'hover:bg-slate-50 dark:hover:bg-midnight-card/50'}`}>
                ...
                >
                  ...
                  <span className={`text-base font-bold truncate ${isParentVisuallyChecked ? 'text-indigo-800 dark:text-indigo-100' : 'text-slate-700 dark:text-slate-200'}`}>{style.label}</span>
