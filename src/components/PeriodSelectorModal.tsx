...
const PeriodSelectorModal: React.FC<PeriodSelectorModalProps> = ({
  ...
  return (
    ...
    >
      <div
        className={`bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-sm transform transition-all duration-300 ease-in-out ${isAnimating ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`}
      ...
      >
        ...
        <div className="p-4">
          ...
          <div className="space-y-4">
            {activeTab === 'daily' && (
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Seleziona Giorno</label>
                <input
                  type="date"
                  value={selectedDate}
                  onChange={(e) => setSelectedDate(e.target.value)}
                  className="block w-full rounded-xl border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 border p-3 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:outline-none transition-colors"
                />
              </div>
            )}
            ...
            {activeTab === 'weekly' && (
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Seleziona un giorno della settimana</label>
                <input
                  type="date"
                  value={selectedDate}
                  onChange={(e) => setSelectedDate(e.target.value)}
                  className="block w-full rounded-xl border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 border p-3 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:outline-none transition-colors"
                />
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">Verrà mostrata l'intera settimana che include questo giorno.</p>
              </div>
            )}
            ...
            {activeTab === 'yearly' && (
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1 transition-colors">Seleziona Anno</label>
                ...
                <div className="flex gap-2 items-center">
                  <button
                    ...
                    className="p-3 bg-slate-100 dark:bg-slate-800 rounded-lg font-bold text-slate-600 dark:text-slate-400 transition-colors"
                  >-</button>
                  <div className="flex-1 text-center p-3 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl font-bold text-lg text-slate-800 dark:text-slate-100 transition-colors">
            ...
            {activeTab === 'custom' && (
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Dal</label>
                  <input
                    type="date"
                    value={customStart}
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Al</label>
                  <input
                    type="date"
                    value={customEnd}
        ...
        <div className="p-4 border-t border-slate-100 dark:border-slate-800 flex justify-end transition-colors">
          <button
            onClick={handleApply}
