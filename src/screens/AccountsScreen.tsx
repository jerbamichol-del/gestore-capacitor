        ...
        <div className={`relative rounded-lg overflow-hidden border border-slate-100 dark:border-slate-800 mb-2 transition-colors duration-200 ${bgClass}`}>
            ...
            >
                <div className="flex items-center gap-3">
                    {isSelected ? (
                        <div className="w-8 h-8 rounded-xl flex items-center justify-center bg-indigo-600 dark:bg-indigo-500 text-white flex-shrink-0">
                            <CheckIcon className="w-5 h-5" strokeWidth={3} />
                        </div>
                    ) : (
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${isIncoming ? 'bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400' : 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400'}`}>
                            <ArrowRightIcon className={`w-4 h-4 transform ${isIncoming ? 'rotate-180' : ''}`} />
                        </div>
                    )}
                    <div>
                        <p className={`text-sm font-semibold ${isSelected ? 'text-indigo-900 dark:text-indigo-100' : 'text-slate-800 dark:text-slate-100'}`}>
                ...
                <span className={`font-bold ${isSelected ? 'text-indigo-900 dark:text-indigo-100' : isIncoming ? 'text-green-600 dark:text-green-400' : 'text-slate-700 dark:text-slate-200'}`}>
...
const AccountsScreen: React.FC<AccountsScreenProps> = ({ onClose }) => {
    ...
    // Replace onClose with handleClose in JSX
    ...
    return (
        <div className="fixed inset-0 z-50 bg-slate-100 dark:bg-slate-950 flex flex-col animate-fade-in-up transition-colors">
            <header className="sticky top-0 z-20 flex items-center gap-4 p-4 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm shadow-sm h-[60px] transition-colors">
                {isSelectionMode && !editingAccountId ? (
                    <>
                        ...
                        <h1 className="text-xl font-bold text-indigo-800 dark:text-indigo-400 flex-1">{selectedTransferIds.size} Selezionati</h1>
                    ...
                    <>
                        ...
                        <h1 className="text-xl font-bold text-slate-800 dark:text-slate-100">I Miei Conti</h1>
                    </>
                )}
            </header>
            ...
            <main className="flex-1 overflow-y-auto p-4 space-y-6">
                ...
                <div
                    ...
                    className={`bg-indigo-600 dark:bg-indigo-700 rounded-2xl p-6 text-white shadow-lg shadow-indigo-200 dark:shadow-indigo-950/50 transition-all duration-300 ${!isSelectionMode ? 'cursor-pointer active:scale-[0.98]' : ''}`}
                >
                    ...
                    {accounts.map(acc => {
                        ...
                        return (
                            <div
                                ...
                                className={`bg-white dark:bg-slate-900 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 flex items-center justify-between transition-all ${!isSelectionMode ? 'active:scale-[0.98] cursor-pointer' : 'opacity-50'}`}
                            >
            ...
            {editingAccountId && (
                ...
                >
                    <div className="flex justify-between items-center p-4 border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 sticky top-0 z-40 transition-colors">
                        {isSelectionMode ? (
                            <>
                                <button onClick={handleCancelSelection} className="p-2 -ml-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400 transition-colors">
                                    <ArrowLeftIcon className="w-6 h-6" />
                                </button>
                                <h3 className="font-bold text-lg text-indigo-800 dark:text-indigo-400 flex-1 ml-2">{selectedTransferIds.size} Selezionati</h3>
                                <button onClick={handleBulkDeleteClick} className="p-2 rounded-full hover:bg-red-50 dark:hover:bg-red-900/30 text-red-600 dark:text-red-400 transition-colors">
                                    <TrashIcon className="w-6 h-6" />
                                </button>
                            </>
                        ) : (
                            <>
                                <button onClick={handleModalClose} className="p-2 -ml-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400 transition-colors">
                                    <XMarkIcon className="w-6 h-6" />
                                </button>
                                <h3 className="font-bold text-lg text-slate-800 dark:text-slate-100 flex-1 ml-4 text-left">
                                    {editingAccount?.name}
                                </h3>
                                <div className="relative">
                                    <button
                                        ...
                                        className={`p-2 rounded-full transition-colors ${isFilterActive ? 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400' : 'hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400'}`}
                                        aria-label="Ordina e Filtra"
                                    >
                                        <ArrowsUpDownIcon className="w-6 h-6" />
                                    </button>
                                    {isSortMenuOpen && (
                                        <div ref={sortMenuRef} className="absolute top-full right-0 mt-2 w-52 bg-white dark:bg-slate-900 rounded-lg shadow-xl border border-slate-100 dark:border-slate-800 z-50 overflow-hidden animate-fade-in-up" onPointerDown={(e) => e.stopPropagation()}>
                                            <div className="py-2">
                                                <p className="px-4 py-1 text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Ordina per</p>
                                                <button onClick={() => handleSortSelect('date')} className={`w-full text-left px-4 py-2.5 text-sm font-semibold flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800 ${sortOption === 'date' ? 'text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30' : 'text-slate-700 dark:text-slate-300'}`}><span>Data (Predefinito)</span>{sortOption === 'date' && <CheckIcon className="w-4 h-4" />}</button>
                                                <button onClick={() => handleSortSelect('amount-desc')} className={`w-full text-left px-4 py-2.5 text-sm font-semibold flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800 ${sortOption === 'amount-desc' ? 'text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30' : 'text-slate-700 dark:text-slate-300'}`}><span>Importo (Decrescente)</span>{sortOption === 'amount-desc' && <CheckIcon className="w-4 h-4" />}</button>
                                                <button onClick={() => handleSortSelect('amount-asc')} className={`w-full text-left px-4 py-2.5 text-sm font-semibold flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800 ${sortOption === 'amount-asc' ? 'text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30' : 'text-slate-700 dark:text-slate-300'}`}><span>Importo (Crescente)</span>{sortOption === 'amount-asc' && <CheckIcon className="w-4 h-4" />}</button>
                                                <div className="border-t border-slate-100 dark:border-slate-800 my-2"></div>
                                                <p className="px-4 py-1 text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Filtra per</p>
                                                <button onClick={() => handleFilterSelect('all')} className={`w-full text-left px-4 py-2.5 text-sm font-semibold flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800 ${filterOption === 'all' ? 'text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30' : 'text-slate-700 dark:text-slate-300'}`}><span>Tutti</span>{filterOption === 'all' && <CheckIcon className="w-4 h-4" />}</button>
                                                <button onClick={() => handleFilterSelect('incoming')} className={`w-full text-left px-4 py-2.5 text-sm font-semibold flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800 ${filterOption === 'incoming' ? 'text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30' : 'text-slate-700 dark:text-slate-300'}`}><span>Solo Entrate</span>{filterOption === 'incoming' && <CheckIcon className="w-4 h-4" />}</button>
                                                <button onClick={() => handleFilterSelect('outgoing')} className={`w-full text-left px-4 py-2.5 text-sm font-semibold flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800 ${filterOption === 'outgoing' ? 'text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30' : 'text-slate-700 dark:text-slate-300'}`}><span>Solo Uscite</span>{filterOption === 'outgoing' && <CheckIcon className="w-4 h-4" />}</button>
                                            </div>
                                        </div>
                                    )}
                                </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-6 transition-colors" onClick={() => setOpenTransferId(null)}>
                        <div className="space-y-6">
                            <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-2xl text-center border border-slate-200 dark:border-slate-800 transition-colors">
                                <p className="text-xs text-slate-500 dark:text-slate-400 uppercase font-bold tracking-wide mb-1">Saldo Attuale</p>
                                <p className="text-3xl font-bold text-slate-800 dark:text-slate-100 transition-colors">{formatCurrency(accountBalances[editingAccountId] || 0)}</p>
                            </div>
                            ...
                            <div className={`flex gap-3 pt-2 ${isSelectionMode ? 'opacity-50 pointer-events-none' : ''}`}>
                                <button
                                    ...
                                    className="flex-1 py-3 text-slate-700 dark:text-slate-200 font-bold bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl transition-colors"
                                >
                                    Annulla
                                </button>
                                <button
                            ...
                            <div className="pt-6 border-t border-slate-100 dark:border-slate-800 transition-colors">
                                <div className="flex items-center justify-between mb-4">
                                    <h4 className="text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Storico Trasferimenti</h4>
                                    {isFilterActive && (
                                        <span className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/30 px-2 py-1 rounded-full">
                                            Filtro attivo
                                        </span>
                                    )}
                                </div>
