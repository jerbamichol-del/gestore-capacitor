...
const RecurringExpensesScreen: React.FC<RecurringExpensesScreenProps> = ({ onClose, onCloseStart }) => {
  const { recurringExpenses, expenses, accounts, deleteRecurringExpenses: onDeleteRecurringExpenses, handleDeleteRequest: onDeleteRequest } = useTransactions();
  const { setEditingRecurringExpense, setIsAddModalOpen } = useUI();
  ...
  const onDelete = onDeleteRequest; // Map context function
      ...
      <header className="sticky top-0 z-20 flex items-center gap-4 p-4 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm shadow-sm h-[60px] transition-colors">
        {isSelectionMode ? (
          <>
            ...
            <button onClick={handleBulkDeleteClick} className="p-2 rounded-full hover:bg-red-100 dark:hover:bg-red-900/40 text-red-600 dark:text-red-400 transition-colors" aria-label="Elimina selezionati"><TrashIcon className="w-6 h-6" /></button>
          </>
        ) : (
          <>
            <button onClick={handleClose} className="p-2 rounded-full hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors" aria-label="Indietro"><ArrowLeftIcon className="w-6 h-6 text-slate-700 dark:text-slate-200" /></button>
            <h1 className="text-xl font-bold text-slate-800 dark:text-slate-100 flex-1">Spese Programmate</h1>
          </>
        )}
      </header>
      <main className="overflow-y-auto h-[calc(100%-60px)] p-2 transition-colors" style={{ touchAction: 'pan-y' }}>
        {sortedExpenses.length > 0 ? (
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-md overflow-hidden my-4 transition-colors">
            {sortedExpenses.map((expense, index) => (
              <React.Fragment key={expense.id}>
                {index > 0 && <hr className="border-t border-slate-200 dark:border-slate-800 ml-16 transition-colors" />}
          ...
          <div className="text-center text-slate-500 dark:text-slate-400 pt-20 px-6 transition-colors">
