...
const RecurringExpenseItem: React.FC<{
  ...
  return (
    <div className={`relative overflow-hidden transition-colors duration-200 select-none ${isSelected ? 'bg-sunset-peach/30 dark:bg-electric-violet/20 ring-1 ring-inset ring-sunset-coral/30 dark:ring-electric-violet/40' : isFinished ? 'bg-slate-50/50 dark:bg-midnight-card/40 opacity-75' : 'bg-white/60 dark:bg-midnight-card/60'}`} >
      ...
      <div ref={itemRef} onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerCancel={handlePointerCancel} onClick={handleClick} className={`relative flex items-center gap-4 py-3 px-4 ${isSelected ? 'bg-sunset-peach/30 dark:bg-electric-violet/20' : isFinished ? 'bg-slate-50/50 dark:bg-midnight-card/40' : 'bg-white/60 dark:bg-midnight-card/60'} z-10 cursor-pointer transition-colors duration-200 select-none`} style={{ touchAction: 'pan-y' }}>
...
const RecurringExpensesScreen: React.FC<RecurringExpensesScreenProps> = ({ recurringExpenses, expenses, accounts, onClose, onCloseStart, onEdit, onDelete, onDeleteRecurringExpenses }) => {
  ...
  return (
    <div className={`fixed inset-0 z-50 bg-white/50 backdrop-blur-strong dark:bg-midnight transform transition-transform duration-300 ease-in-out ${isAnimatingIn ? 'translate-y-0' : 'translate-y-full'}`} style={{ touchAction: 'pan-y' }} onClick={() => { if (openItemId) setOpenItemId(null); }} {...tapBridge}>
      <header className="sticky top-0 z-20 flex items-center gap-4 p-4 midnight-card shadow-sm dark:shadow-electric-violet/5 h-[60px] border-b border-transparent dark:border-electric-violet/10 transition-colors">
        {isSelectionMode ? (
          <>
            ...
            <button onClick={handleBulkDeleteClick} className="p-2 rounded-full hover:bg-red-100/50 dark:hover:bg-rose-500/20 text-red-600 dark:text-rose-400 transition-colors" aria-label="Elimina selezionati"><TrashIcon className="w-6 h-6" /></button>
          </>
        ) : (
          <>
