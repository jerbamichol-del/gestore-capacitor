...
const RecurringExpenseItem: React.FC<{
  ...
  return (
    <div className={`relative overflow-hidden transition-colors duration-200 select-none ${isSelected ? 'bg-sunset-peach/30 dark:bg-electric-violet/20 ring-1 ring-inset ring-sunset-coral/30 dark:ring-electric-violet/40' : isFinished ? 'bg-[#FFF8F0] dark:bg-midnight-card/40 opacity-75' : 'bg-[#FFF8F0] dark:bg-midnight-card/60'}`} >
      ...
      <div ref={itemRef} onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerCancel={handlePointerCancel} onClick={handleClick} className={`relative flex items-center gap-4 py-3 px-4 ${isSelected ? 'bg-sunset-peach/30 dark:bg-electric-violet/20' : isFinished ? 'bg-[#FFF8F0] dark:bg-midnight-card/40' : 'bg-[#FFF8F0] dark:bg-midnight-card/60'} z-10 cursor-pointer transition-colors duration-200 select-none`} style={{ touchAction: 'pan-y' }}>
...
const RecurringExpensesScreen: React.FC<RecurringExpensesScreenProps> = ({ recurringExpenses, expenses, accounts, onClose, onCloseStart, onEdit, onDelete, onDeleteRecurringExpenses }) => {
  ...
  return (
    <div className={`fixed inset-0 z-50 bg-[#FFF8F0] dark:bg-midnight transform transition-transform duration-300 ease-in-out overscroll-contain ${isAnimatingIn ? 'translate-y-0' : 'translate-y-full'}`} style={{ touchAction: 'pan-y' }} onClick={() => { if (openItemId) setOpenItemId(null); }} {...tapBridge}>
