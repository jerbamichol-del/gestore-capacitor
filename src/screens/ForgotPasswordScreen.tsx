...
const ForgotPasswordScreen: React.FC<ForgotPasswordScreenProps> = ({ onBackToLogin, onRequestSent }) => {
    ...
    const inputStyles = "block w-full rounded-md border border-slate-300 dark:border-electric-violet/30 bg-sunset-cream/60 dark:bg-midnight-card/50 py-2.5 pl-10 pr-3 text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:border-indigo-500 dark:focus:border-electric-violet focus:ring-1 focus:ring-indigo-500 dark:focus:ring-electric-violet sm:text-sm transition-colors";

    return (
        <AuthLayout>
            <div className="text-center">
                <h2 className="text-xl font-bold text-sunset-text dark:text-white mb-2">Reimposta PIN</h2>
                <>
                    ...
                    <button
                        ...
                        className="mt-6 w-full text-center text-sm font-semibold text-sunset-coral dark:text-electric-violet hover:text-sunset-pink dark:hover:text-electric-pink"
                    >
                        Annulla
                    </button>
                </>
